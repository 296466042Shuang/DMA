...
class o11dma(object):
    ...
    def calculate_phase(self,ax,deg=3):
        t = self.getChannel('Time')
        x = self.getChannel('Indentation')
        y = self.getChannel('Load')
        for i in range(len(self.start)):
            ...
            ext_load = np.interp(time, st, resample_load)
            ext_ind = np.interp(time, st, resample_ind)
            ax[i].plot(st,resample_load,'-o')
            ax[i].plot(time,ext_load,'x')
            kmax=0
            delta=time[1]-time[0]
            ...
            print(np.pi*delta*kmax/semiperiod)
        plt.show()

    def calculate_amplitude(self,ax,deg=3):
        t = self.getChannel('Time')
        x = self.getChannel('Indentation')
        y = self.getChannel('Load')
        z = self.getChannel('Piezo')
        for i in range(len(self.start)):
            start=self.start[i]
            stop =self.stop[i]
            istart = np.argmin((t -start)**2)
            ...
            #Piezo
            pointsUpPiezo=self.find_peaks(st,sz,freq)
            pointsDownPiezo=self.find_peaks(st,-sz,freq)
            p1Piezo=np.polynomial.Polynomial.fit(st[pointsUp],sz[pointsUpPiezo],deg)
            p2Piezo=np.polynomial.Polynomial.fit(st[pointsDown], sz[pointsDownPiezo], deg)
            dt = t[1] - t[0]
            semiperiod = 1 / freq / 2
            iquad = int(semiperiod / dt)
            ...
            corrected_load_smooth = savgol_filter(corrected_load, window, 3)
            corrected_ind_smooth = savgol_filter(sx, window, 3)
            corrected_Piezo_smooth = savgol_filter(sz,window,3)

            self.A_load = (max(corrected_load_smooth) - min(corrected_load_smooth))
            self.A_ind = (max(corrected_ind_smooth) - min(corrected_ind_smooth))

            resample_load = -1 + 2 * (corrected_load_smooth - min(corrected_load_smooth)) / (
                        max(corrected_load_smooth) - min(corrected_load_smooth))
            resample_ind = -1 + 2 * (corrected_ind_smooth - min(corrected_ind_smooth)) / (
                        max(corrected_ind_smooth) - min(corrected_ind_smooth))
            resample_piezo= -1 + 2 * (corrected_Piezo_smooth - min(corrected_Piezo_smooth)) / (
                        max(corrected_Piezo_smooth) - min(corrected_Piezo_smooth))

            time = np.linspace(np.min(st), np.max(st), len(st))
            piezo = np.linspace(np.min(sz),np.max(sz),len(sz))
            cmax = 0
            'Linear interpolation'
            ext_load = np.interp(time, st, resample_load)
            ext_ind = np.interp(time, st, resample_ind)
            ext_piezo = np.interp(time,st,resample_piezo)
            kmax = 0
            delta = time[1] - time[0]
            for k in range(iquad):
                corr = np.sum(ext_load[:-iquad] * ext_piezo[k:-iquad + k])
                if corr > cmax:
                    cmax = corr
                    kmax = k
            self.amplitude.append(np.pi * delta * kmax / semiperiod)  # phase is in radiants!!!
            print(np.pi * delta * kmax / semiperiod)

            # 'freq'
            fft_size=len(sz)
            tstemp = np.linspace(0, int(1e6 / freq * fft_size), fft_size) / 1e3
            # tstemp=np.linspace(np.min(sz),np.max(sz),len(sz))
            # print(tstemp)
            #
            # Drawing graphics
            # Plotting raw data trends
            #plt.figure(figsize=(14, 8))
            # plt.title("data")
            # plt.ylabel("sample value")
            # plt.xlabel("time(ms)")
            # plt.plot(tstemp, sz[:fft_size])
            # #plt.plot(st, tstemp)
            # # plt.show()
            # # # Plotting histograms of the original data distribution
            # # #plt.figure(figsize=(14, 8))
            # plt.title("data")
            # plt.ylabel("spot")
            # plt.xlabel("sample value")
            # plt.hist(x[:fft_size], bins=100)
            # # plt.show()
            #
            ...
            #ax[i].figure(figsize=(14, 8))
            ax[i] = plt.subplot(211)
            ax[i].set_xlabel('time [s]')
            ax[i].set_ylabel('sample value')
            n = np.arange(fft_size) / fft_size
            ax[i].plot(n, sz)
            ax[i] = plt.subplot(212)
            ax[i].set_xscale('log')
            ax[i].set_xlabel('frequency [Hz]')
            ax[i].set_ylabel('|amplitude|')
            ax[i].plot(rf)
            #ax[i].legend()
            #plt.savefig('sample%d-graph.png'%(i))
        plt.show()
